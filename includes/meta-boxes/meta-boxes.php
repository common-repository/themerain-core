...
class ThemeRain_Meta_Boxes {
	...
	public function field( $post, $field ) {
		...
		if ( isset( $field['cond'] ) ) {
			...
			$cond_val  = explode( '|', $field['cond'][1] );
			$cond      = ' data-condition="' . implode( ',', $field['cond'] ) . '"';
			$class[]   = ( ( '1' === $field['cond'][1] && $cond_meta ) || ( '0' === $field['cond'][1] && ! $cond_meta ) || $cond_meta === $field['cond'][1] || in_array( $cond_meta, $cond_val ) ) ? '' : 'themerain-hidden';
		}
